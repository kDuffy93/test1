...
(function () {
                ...
                if (link instanceof HTMLElement) {
                    if (link.dataset.linkid) {
                        link.addEventListener("click", function (event) {
                            event.preventDefault();
                            document.title = link.dataset.linkid;
                            //change url
                            history.pushState({}, "", "/" + document.title);
                            $("li>a").each(function () {
                                $(this).removeClass("active");
    ...
    async function populateAboutPage() {
        ...
        let aboutContentSection = document.querySelector("#aboutContent>article:last-of-type>p");
        aboutContentSection.textContent = aboutContent;
        //for the logo
        let logo = document.querySelector("#aboutContent>article:last-of-type>img");
        logo.src = "./Assets/personalLogo.png";
        //for the main photo
        let photo = document.querySelector("#aboutContent>article:first-of-type>img");
    ...
    async function populateservicesPage() {
        ...
        for (const service in services) {
            if (Object.prototype.hasOwnProperty.call(services, service)) {
                ...
                await $.get(`./views/components/servicesCards.html`, function (html_data1) {
                ...
                let textBox = document.getElementById("temptext");
                textBox.textContent = element.name;
                //change new card id'd
                let card = document.getElementById("tempsection");
    ...
    async function populateProjectsPage() {
        ...
        for (const project in projects) {
            if (Object.prototype.hasOwnProperty.call(projects, project)) {
                ...
                await $.get(`./views/components/projectCard.html`, function (html_data1) {
                ...
                let url = document.getElementById("tempurl");
                url.href = element.URL;
                let iframeContainer = document.getElementById("iframeContainer");
                let iframe = document.createElement("iframe");
                iframe.src = element.URL;
                iframeContainer.appendChild(iframe);
                let name = document.getElementById("tempname");
                name.textContent = element.Title;
                let repo = document.getElementById("temprepo");
                repo.href = element.repo;
                let desc = document.getElementById("tempdescription");
    ...
    let LoadContentBackwards = (backLink) => {
